...
ws.onmessage = async (event) => {
  ...
  new SlashCommandBuilder()
    .setName('stats')
    .setDescription('Get stats or heatmaps for a map')
    .addStringOption(option =>
      option.setName('map')
        .setDescription('Map name (case-sensitive)')
        .setRequired(true)
    ...
    .addStringOption(option =>
      option.setName('display')
        .setDescription('Display mode (optional, e.g. heatmap)')
        .setRequired(false)
    )
    .addStringOption(option =>
      option.setName('gamemodes')
        .setDescription('Game modes filter, comma-separated (optional)')
        .setRequired(false)
    ),
  async (interaction) => {
    const mapName = interaction.options.getString('map');
    ...
    const gameModes = interaction.options.getString('gamemodes');

    await interaction.deferReply();
    await interaction.editReply("📡 Connecting to server...");

    // Use your backend WS endpoint here
    const ws = new WebSocket("wss://tktk123456-openfrontio-51.deno.dev/ws");
    ...
    ws.onmessage = async (event) => {
      ...
      if (data.type === "progress") {
        const content = (() => {
          if (data.task === "filterGames") {
            return `🔄 Map Progress: ${data.progress}% (${data.currentCount}/${data.total} checked, ${data.matchesCount} matches)`;
          } else if (data.task === "getStats") {
            return `🔄 ${data.statType.charAt(0).toUpperCase() + data.statType.slice(1).toLowerCase()} Stat Progress: Game ${data.currentGame}/${data.totalGames}, Intents processed: ${data.currentIntents}, Tracked entries: ${data.tracked}`;
          } else {
            return `🔄 Progress (${data.task}): ${data.progress}% (${data.currentCount} checked)`;
          }
        })();
      ...
      if (data.done) {
        ...
        if (data.display === "heatmap" && data.heatmap) {
          // Send heatmap summary as text, no canvas support in Discord
          await interaction.editReply(
            `✅ Heatmap stats for ${mapName} (${statType}): ${data.stats?.matchingGameModes ?? 'N/A'} games with valid data.\n(Note: Full heatmap image display is not supported in Discord.)`
          );
        } else if (data.matches?.length) {
          const trimmed = data.matches.slice(0, 20);
          await interaction.editReply(
            `✅ Found ${data.matches.length} games on **${mapName}**.\nFirst few:\n\`\`\`json\n${JSON.stringify(trimmed, null, 2)}\n\`\`\``
          );
        } else if (data.stats) {
          await interaction.editReply(`✅ Stats for **${mapName}**:\n\`\`\`json\n${JSON.stringify(data.stats, null, 2)}\n\`\`\``);
        } else {
          await interaction.editReply("✅ Done, but no results returned.");
        }
        ws.close();
      }
